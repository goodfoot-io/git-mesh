...
# Idempotent: keys, port, and authorized_keys persist across re-runs.
...
OUT_DIR="/workspace/scripts/ssh"
OUT_SCRIPT="$OUT_DIR/configure-external.sh"
...
# $ALIAS at $HOST_IP:$PORT as user 'node'.
set -euo pipefail
...
grep -qxF "\$HOST_LINE" "\$KNOWN" || echo "\$HOST_LINE" >> "\$KNOWN"
...
echo "Configured: ssh $ALIAS  ->  node@$HOST_IP:$PORT"
EOF
chmod +x "$OUT_SCRIPT"
