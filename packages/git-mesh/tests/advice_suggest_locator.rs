...
use git_mesh::advice::suggest::{attach_locators, prior_context_atoms, Op, OpKind, SuggestConfig};
...
fn edit_attaches_to_nearest_prior_ranged_read() {
    ...
    assert_eq!(ops[2].inferred_start, Some(10), "must attach to nearer read B");
...
fn edit_with_no_prior_read_in_window_is_unanchored() {
    ...
    assert!(edit.inferred_start.is_none(), "out-of-window edit must be unanchored");
}
...
fn directory_penalty_reduces_score_for_cross_dir_attachment() {
    ...
    attach_locators(&mut ops, &cfg());
    assert_eq!(ops[0].locator_forward, Some(true), "forward read must be flagged");
    assert!(ops[0].inferred_start.is_some(), "forward read is still attached when it's the only candidate");
}
...
fn same_directory_attachment_has_no_penalty() {
    ...
    attach_locators(&mut ops, &cfg());
    assert_eq!(ops[1].locator_forward, Some(false), "backward read must be preferred");
...
fn prior_context_k_limits_candidates_considered() {
    ...
    assert_eq!(atoms.len(), 2, "only locator_prior_context_k=2 atoms returned");
