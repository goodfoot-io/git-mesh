...
fn snapshot_creates_baseline_and_last_flush_state() -> Result<()> {
    ...
    assert!(dir.join("baseline.state").exists(), "baseline.state must exist");
    assert!(dir.join("last-flush.state").exists(), "last-flush.state must exist");
    assert!(dir.join("baseline.objects").is_dir(), "baseline.objects/ must exist");
    assert!(dir.join("last-flush.objects").is_dir(), "last-flush.objects/ must exist");
...
fn read_appends_to_reads_jsonl() -> Result<()> {
    ...
    assert_eq!(n_lines, 2, "reads.jsonl must record one line per `read`, got:\n{content}");
...
fn render_advances_last_flush_state_and_records_touch() -> Result<()> {
    ...
    assert_ne!(last_flush_after, last_flush_before, "last-flush.state must advance");
    let touches = std::fs::read_to_string(dir.join("touches.jsonl"))?;
    assert!(!touches.is_empty(), "touches.jsonl must record a non-empty render");
    Ok(())
}
...
fn fails_closed_when_baseline_missing_for_read() -> Result<()> {
    ...
    assert!(!out.status.success(), "read without snapshot must fail closed");
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("snapshot"), "stderr must name `snapshot`: {stderr}");
    Ok(())
}
