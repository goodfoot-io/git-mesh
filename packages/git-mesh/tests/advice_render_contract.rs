...
// ---------------------------------------------------------------------------
// snapshot, then milestone with no changes: silent, exit 0.
// ---------------------------------------------------------------------------
#[test]
fn milestone_after_snapshot_no_changes_is_silent() -> Result<()> {
    let repo = TestRepo::seeded()?;
    let sid = session_id("noop");
    let _ = run_advice(&repo, &sid, &["snapshot"])?;
    let out = run_advice(&repo, &sid, &["milestone"])?;
    assert!(
        out.status.success(),
...
// ---------------------------------------------------------------------------
// milestone before snapshot: non-zero, message names `snapshot`.
// ---------------------------------------------------------------------------
#[test]
fn milestone_without_snapshot_fails_closed() -> Result<()> {
    let repo = TestRepo::seeded()?;
...
// ---------------------------------------------------------------------------
// milestone advances read_cursor even when nothing prints.
// ---------------------------------------------------------------------------
#[test]
fn milestone_advances_read_cursor_when_silent() -> Result<()> {
    let repo = TestRepo::seeded()?;
    ...
    assert_eq!(
        ...
        "read_cursor must equal byte length of reads.jsonl after milestone"
    );
    Ok(())
}

// ---------------------------------------------------------------------------
// milestone records a touch interval when delta non-empty.
// ---------------------------------------------------------------------------
#[test]
fn milestone_records_touch_when_delta_nonempty() -> Result<()> {
    let repo = TestRepo::seeded()?;
...
// ---------------------------------------------------------------------------
// Two consecutive milestones: second diffs against first milestone's tree.
// ---------------------------------------------------------------------------
#[test]
fn two_consecutive_milestones_diff_against_last_flush() -> Result<()> {
    let repo = TestRepo::seeded()?;
    ...
    let out1 = run_advice(&repo, &sid, &["milestone"])?;
    assert!(out1.status.success());

    let dir = session_dir(&repo, &sid);
    // After first milestone last-flush.objects must exist.
    assert!(
        dir.join("last-flush.objects").is_dir(),
        "last-flush.objects must exist after first milestone"
    );

    // Modify B (a different file).
    repo.write_file("file2.txt", "B1\n")?;
    let touches_before = std::fs::metadata(dir.join("touches.jsonl"))?.len();
    let out2 = run_advice(&repo, &sid, &["milestone"])?;
    assert!(
        out2.status.success(),
        "stderr={}",
    ...
    let touches_after = std::fs::metadata(dir.join("touches.jsonl"))?.len();
    // The second milestone saw a non-empty incr_delta (file2 changed since last
    // flush) and so must have recorded a fresh touch interval.
    assert!(
        touches_after > touches_before,
        "second milestone must record a touch (B's change vs last-flush)"
    );
    Ok(())
}
...
// after a successful milestone with delta, last-flush.state advanced and
// last-flush.objects/ exists — which is the load-bearing invariant.
// ---------------------------------------------------------------------------
#[test]
fn milestone_advances_last_flush_state_before_stdout() -> Result<()> {
    let repo = TestRepo::seeded()?;
    ...
    // Make a change and run milestone.
    ...
    assert_ne!(
        ...
        "last-flush.state must advance after a milestone with delta"
    );
    assert!(dir.join("last-flush.objects").is_dir());
    // current.objects-* must NOT linger after a successful milestone.
...
// ---------------------------------------------------------------------------
// Finding 3: touches.jsonl records real paths after a milestone with delta+reads.
// ---------------------------------------------------------------------------
#[test]
fn touches_carry_real_paths_after_milestone() -> Result<()> {
    let repo = TestRepo::seeded()?;
...
// ---------------------------------------------------------------------------
// milestone error path leaves no current.objects-<uuid> behind.
// We trigger an error by corrupting last-flush.state so read_last_flush fails
// AFTER snapshot.
// ---------------------------------------------------------------------------
#[test]
fn milestone_error_cleans_up_current_objects() -> Result<()> {
    let repo = TestRepo::seeded()?;
    ...
    assert!(
        !out.status.success(),
        "expected milestone to fail with corrupt last-flush.state"
    );
...
fn stale_last_flush_state_falls_back_to_baseline() -> Result<()> {
    ...
    assert!(
        out.status.success(),
        "milestone must recover from stale last-flush.state, stderr={}",
        String::from_utf8_lossy(&out.stderr)
    );
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
fn stale_last_flush_via_alternates_triggers_fallback() -> Result<()> {
    ...
    // First snapshot + milestone so last-flush.objects/ is populated with a
    // real tree object.
    run_advice(&repo, &sid, &["snapshot"])?;
    ...
    // in run_advice_milestone passes) but leave it empty — the tree cannot be
    // found inside it.
    ...
    assert!(
        out.status.success(),
        "milestone must fall back gracefully; stderr={}",
        String::from_utf8_lossy(&out.stderr)
    );
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
// ---------------------------------------------------------------------------
// Successful milestone advances last-flush.state (seen-sets and cache
// correctness invariant).
// ---------------------------------------------------------------------------
#[test]
fn successful_milestone_advances_last_flush_state() -> Result<()> {
    let repo = TestRepo::seeded()?;
