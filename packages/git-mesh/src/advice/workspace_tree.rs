...
pub fn capture(repo: &gix::Repository, objects_dir: &Path) -> Result<WorkspaceTree> {
    ...
    if real_index.exists() {
        std::fs::copy(&real_index, &temp_index)
            .with_context(|| format!("copy index `{}` -> `{}`", real_index.display(), temp_index.display()))?;
    } else {
...
pub fn diff_trees(
    ...
    let alt_string = std::env::join_paths(alts.iter())
        .context("join alternate object dirs")?;
...
fn parse_raw_diff_z(bytes: &[u8]) -> Result<Vec<DiffEntry>> {
    ...
    while i < bytes.len() {
        ...
        let nul = bytes[i..].iter().position(|&b| b == 0)
            .ok_or_else(|| anyhow::anyhow!("malformed raw diff: missing NUL"))?;
        let header = std::str::from_utf8(&bytes[i..i + nul])
            .context("raw diff header utf8")?;
        i += nul + 1;
        ...
        let entry = match status_byte {
            ...
            'R' => {
                ...
                let nul = bytes[i..].iter().position(|&b| b == 0)
                    .ok_or_else(|| anyhow::anyhow!("malformed raw diff: missing rename target NUL"))?;
                let path2 = std::str::from_utf8(&bytes[i..i + nul])
                    .context("raw diff rename target utf8")?
                    .to_string();
                i += nul + 1;
                let score: u8 = status[1..].parse().unwrap_or(100);
                DiffEntry::Renamed { from: path1, to: path2, score }
            }
            'C' => {
                // Copy: skip second path; treat as Added of dst.
                let nul = bytes[i..].iter().position(|&b| b == 0)
                    .ok_or_else(|| anyhow::anyhow!("malformed raw diff: missing copy target NUL"))?;
                let path2 = std::str::from_utf8(&bytes[i..i + nul])
